...
# ==========================================
# CIFAR-10（WRN-28-10 + クリーン・ファインチュン実装版）
...
#
# パス解決:
# - ローカル: 既定は本ファイルから見たこの課題のプロジェクトルート
#   データは data/{x_train,t_train,x_test}.npy または data/input/ 配下
# - 環境変数 CIFAR10_WORK_DIR で作業ディレクトリを上書き可能
# - Google Colab: google.colab が使える場合のみ Drive マウントにフォールバック
# ==========================================
import os
import math
import random
...
def _resolve_work_dir() -> str:
    env = os.environ.get('CIFAR10_WORK_DIR')
    if env:
        return os.path.abspath(env)
    for root in (_PROJECT_ROOT,):
        for sub in ('data', os.path.join('data', 'input')):
            if os.path.isfile(os.path.join(root, sub, 'x_train.npy')):
                return root
    try:
        from google.colab import drive  # type: ignore

        drive.mount('/content/drive', force_remount=True)
        return os.path.abspath('drive/MyDrive/第5回')
    except ImportError:
        return _PROJECT_ROOT


def _resolve_data_dir(work_dir: str) -> str:
    d0 = os.path.join(work_dir, 'data')
    d1 = os.path.join(work_dir, 'data', 'input')
    if os.path.isfile(os.path.join(d0, 'x_train.npy')):
        return d0
    if os.path.isfile(os.path.join(d1, 'x_train.npy')):
        return d1
    return d0


work_dir = _resolve_work_dir()
_data_dir = _resolve_data_dir(work_dir)
...
x_train = np.load(os.path.join(_data_dir, 'x_train.npy'))
t_train = np.load(os.path.join(_data_dir, 't_train.npy'))
x_test = np.load(os.path.join(_data_dir, 'x_test.npy'))
...
_out_dir = os.path.join(work_dir, 'data', 'output')
os.makedirs(_out_dir, exist_ok=True)
out_path = os.path.join(_out_dir, 'submission.csv')
